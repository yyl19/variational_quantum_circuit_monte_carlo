...
push!(LOAD_PATH,"D:\\errormitigation\\VQCMC\\QuantumCircuits","D:\\errormitigation\\VQCMC\\VQC")
using VQC
...
function generatepsi(num)
    ...
    ϕ=(kron(X*X,X)*StateVector(2)-kron(X,X*X)*StateVector(2))/sqrt(2)
    psi=ϕ
    if num>2
        for i=2:num/2
            psi=cat(psi, ϕ)
...
function build_toy_Ansatz(params)
    ...
    for i=1:Nlayer
        for item in Edge
            ...
            push!(circuit, RzGate(item[2], params[counts], isparas=true))
            # push!(circuit, RzGate(item[2], Variable(params[counts])))
            push!(circuit, CNOTGate(control=item[1], target=item[2]))
            #######expyy#####
            counts += 1
            push!(circuit, RxGate(item[1], -pi/2,isparas=false))
            push!(circuit, RxGate(item[2], -pi/2,isparas=false))
            push!(circuit, CNOTGate(control=item[1], target=item[2]))
            push!(circuit, RzGate(item[2], params[counts], isparas=true))
            # push!(circuit, RzGate(item[2], Variable(params[counts])))
            push!(circuit, CNOTGate(control=item[1], target=item[2]))
...
function generPsi_x(THETA)
    ...
    for i in 1:a
        ...
        reset_parameters!(circuit,reparamstoy(THETA[:,i]))
        x=circuit *ψ
        push!(Psi,x)
    end
    return Psi
end
function Psi_xtovec(Psi_x)
    Psia=[]
    for item in Psi_x
        push!(Psia,item.data)
    end
...
global Psi_a=generPsi_x(THETA)#100 quantum states of phi are generated from 100 theta parameters
global Psi_x=Psi_xtovec(Psi_a)
...
global De=0#Defining the barrier of <1>
