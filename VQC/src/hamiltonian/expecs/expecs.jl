...
include("expec_threaded_batch.jl")
include("cu_expec_threaded_batch.jl")
...
#statevectorBatch
function expectation(m::QubitsTerm, state::StateVectorBatch) 
	isempty(m) && return dot(state, state)
	if length(positions(m)) <= LARGEST_SUPPORTED_NTERMS
	    return expectation_value(Tuple(positions(m)), _get_mat(m), storage(state), nqubits(state))
	else
		return dot(state, m * state)
	end
end

expectation_value(pos::Int, m::AbstractMatrix, state::AbstractVector, n::Int) = (length(state) >= 32) ? expectation_value_threaded(
	pos, m, state, n) : expectation_value_serial(pos, m, state)

expectation_value(pos::Tuple{Int}, m::AbstractMatrix, state::AbstractVector, n::Int) = expectation_value(pos[1], m, state, n)

# expectation_value(pos::Int, m::AbstractMatrix, state::AbstractVector, state_c::AbstractVector, n::Int) = (length(state) >= 32) ? expectation_value_threaded(
# 	pos, m, state, state_c) : expectation_value_serial(pos, m, state, state_c)

# expectation_value(pos::Tuple{Int}, m::AbstractMatrix, state::AbstractVector, state_c::AbstractVector, n::Int) = expectation_value(
# 	pos[1], m, state, state_c, n)

#statevectorBatch

##cuda
function expectation(m::QubitsTerm, state::CuStateVectorBatch) 
	isempty(m) && return dot(state, state)
	if length(positions(m)) <= LARGEST_SUPPORTED_NTERMS
	    return expectation_value(Tuple(positions(m)), _get_mat(m), storage(state), nqubits(state))
	else
...
expectation_value(pos::Int, m::AbstractMatrix, state::CuArray, n::Int) = (length(state) >= 32) ? expectation_value_threaded(
	pos, m, state, n) : expectation_value_serial(pos, m, state)

expectation_value(pos::Tuple{Int}, m::AbstractMatrix, state::CuArray, n::Int) = expectation_value(pos[1], m, state, n)
