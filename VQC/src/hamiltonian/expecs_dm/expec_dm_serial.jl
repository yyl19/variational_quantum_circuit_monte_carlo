@inline get_2_by_2(state::AbstractMatrix, posa::Int, posb::Int, stride::Int) = SMatrix{2,2}(state[posa, posa+stride], state[posb, posa+stride], state[posa, posb+stride], state[posb, posb+stride])
...
@adjoint get_2_by_2(state::AbstractMatrix, posa::Int, posb::Int) = get_2_by_2(state::AbstractMatrix, posa::Int, posb::Int), z->(z,nothing,nothing)
...
function _expectation_value_util(pos::Int, m::AbstractMatrix, state::AbstractMatrix)
	...
	s1 = 2^(pos-1)
	s2 = s1 * sj
	r = [Atomic{Float64}(0) for i in 1:L]
	Threads.@threads for i in 0:s2:(size(state)[1]-1)
		for j in 0:(s1-1)
			for k in 0:(L-1)
				pos_start = i + j + 1
				v = get_2_by_2(state, pos_start, pos_start + s1, k*L1)
				@fastmath atomic_add!(r[k+1], real(tr(m * v)))
			end
		end
	end
	return r
