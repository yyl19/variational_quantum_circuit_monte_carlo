...
function conv_net(input::Array{<:Real, 2}, circuit1, m)   # first quantum conv layer
    out1 = circuit1 * input
...
function train_single(depth::Int=9, learn_rate::Real=0.05)
    ...
    loss(c1, ma) = begin
        ...
        for i in 1:length(x_train)
            r += distance(conv_net(x_train[i], c1, ma), y_train[i])
        end
        return r / length(x_train)
    end

    @time r = loss(circuit1, m)
    println("initial loss is $r")

    predict(input::Array{<:Real, 2}) = begin
        r = conv_net(input, circuit1, m)
        return argmax(r)
    end
    ...
    function train(epochs)
        for i in 1:epochs
            @time grad = parameters(gradient(loss, circuit1, m))
#           paras = parameters(circuit)
            Optimise.update!(opt, x0, grad)
            set_parameters!(x0, circuit1, m)
            if i % 1 == 0
